...
protected:
	...
	bool bubble(Rank lo, Rank hi); //交换扫描
...
public:
	...
	Verctor(T const* A, Rank n){copyFrom(A, 0, n);} // 数组整体复制
	...
	Rank insert(T const& e) {return insert(_size, e);} //默认作末元素插入
	void sort(Rank lo, Rank hi); //对[lo, hi)排序
	void sort(){sort(0, _size);} //整体排序 
	void unsort(Rank lo, Rank hi); //对[lo, hi)整乱 
	void unsort(){unsort(0, _size);} //整体排序
	int deduplicate(); //去重
	int uniquify_1(); //有序去重
	int uniquify(); //有序去重
	//遍历
